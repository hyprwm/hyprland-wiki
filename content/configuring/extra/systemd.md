...
Arch Linux has a custom patch in their `xdg-desktop-portal` package to bypass its dependency upon `graphical-session.target`.
However, other distros such as NixOS, don't, and the user needs to start the target manually.

The section below explains how to start `graphical-session.target` with Hyprland.

## hyprland-session.target

This is a minimal way of starting the `graphical-session.target`.
...
hl.on("hyprland.shutdown", function()
    ...
    -- uses a blocking exec function to ensure things have time to close
