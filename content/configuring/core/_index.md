...
When using start-hyprland, arguments to Hyprland are passed after `--`.
Use `--config` (or `-c`) to make Hyprland load specified config.
...
Remove `hl.config({ autogenerated = true })` to remove autogenerated warning.

Configuration is realoaded when changes to files are detected.
To reload manually, use `hyprctl reload`.

> [!WARNING]
> The default config is not complete and does not include every Hyprland options or feature.
> For every configuration option, use the pages linked on this wiki section.

## Language style and syntax

Hyprland uses [Lua](https://lua.org/) for its configuration file.
It's an easy and performant scripting language used throughout various pieces of software.

### General Hyprland Lua tips

#### Example config

Example config file can be found [here](https://github.com/hyprwm/Hyprland/blob/main/example/hyprland.lua).

#### Using Multiple Configuration Files

Configuration can be split across several files.
To load other files, use Lua's [require](#require) function.
Files may be specified either as an absolute path or a path relative to the location of `hyprland.lua`.

In `require`, `.` or `/` can be used as a directory separator.

The `*` wildcard can be used to load multiple files from a directory or to search in all directories at the given path. When wildcard is used path must be in UNIX format.
...
That is, doing `require("nonexistent")` in your main Hyprland config _will kill the execution of your main config_ as described below.
...
Restored functionality may be helpful if you load third-party modules that are broken by Hyprland's error-protection behavior.

#### Autocompletions

Pre-generated Lua stub can be found in `/usr/share/hypr/stubs/` directory. Configure your LSP to include that directory in your Lua workspace to enable autocompletions.
...
If an errors occurs before any of the binds were registered, Hyprland will register a few
emergency keybinds: `SUPER + Q` to open terminal, `SUPER + R` to open hyprlauncher, and `SUPER + M` to exit Hypland, and mention this in the error popup.
