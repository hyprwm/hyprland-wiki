...
| on_focus_under_fullscreen | if there is a fullscreen or maximized window, decide whether a tiled window requested to focus should replace it, stay behind or disable the fullscreen/maximized state. Note: if configured to stay behind, the setting does not apply to the `focuswindow` dispatcher: when requested via `focuswindow`, the window will always be focused. 0 - stay behind (unless via `focuswindow`), 1 - takes over, 2 - unfullscreen/unmaximize [0/1/2] | int | 0 |
